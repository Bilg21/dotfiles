...
def get_keys_and_mouse(groups):
    ...
    for i in groups:
        if i.name == "scratchpad":
            keys.append(Key(mod+ctrl, "minus", lazy.group[i.name].dropdown_toggle("fm")))
            keys.append(Key(mod+ctrl, "equal", lazy.group[i.name].dropdown_toggle("fm2")))
            keys.append(Key(mod+ctrl, "grave", lazy.group[i.name].dropdown_toggle("term")))
        else:
    ...
    keys += [
        ...
        Key(mod, "r",                  lazy.spawn("/usr/bin/dmenu_run -l 10 -i -fn 'Monospace'")),
        Key(mod, "Return",             lazy.spawn(terminal)),
        Key(mod, "Escape",             lazy.spawn("xautolock-force")),
        Key(mod+mswitch, "Escape",     lazy.spawn("xautolock -toggle")),
        Key([], "Print",               lazy.spawn("/usr/bin/gnome-screenshot -i")),
        Key(mod, "s",                  lazy.spawn("/usr/bin/gnome-screenshot")),
        Key(mod+shft, "s",             lazy.spawn("/usr/bin/gnome-screenshot --window")),
        Key(mod+alt, "l",                  lazy.spawn("i3lock -i /home/bilg/Pictures/Wallpapers/wallpaper_mikael_gustafsson.png -e -f -t")),
